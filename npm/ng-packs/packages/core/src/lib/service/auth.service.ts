...
export class AuthService {
  ...
  cookieLogin(userName: string, password: string): Observable<any> {
    return this.httpClient
      .post<IAbpLoginResult>(this.authenticationConfig.cookieConfig.loginApiUrl, {
  ...
  cookieLogout(): Observable<any> {
    return this.httpClient.get<any>(this.authenticationConfig.cookieConfig.logoutApiUrl).pipe(
      map(r => {
  ...
  sessionIsValid(): Observable<boolean> {
    return this.httpClient.get<boolean>(this.authenticationConfig.sessionValidationUrl);
  }
}
